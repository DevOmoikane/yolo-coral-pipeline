...
COPY setup/requirements-ov.txt /requirements-ov.txt
...
# Get OpenVino Model
RUN --mount=type=bind,source=setup/build_ov_model.py,target=/build_ov_model.py \
...
ARG TARGETARCH
RUN --mount=type=bind,source=setup/install_s6_overlay.sh,target=/deps/install_s6_overlay.sh \
...
COPY setup/requirements-dev.txt /requirements-dev.txt

RUN pip3 install -r /requirements.txt

COPY setup/*.tar.gz /pips/
COPY setup/*.whl /pips/
COPY setup/*.zip /pips/
COPY setup/requirements-wheels.txt /requirements-wheels.txt
...
RUN --mount=type=bind,source=setup/install_deps.sh,target=/deps/install_deps.sh \
